...
function TicketInfo() {
  ...
  return (
    <div className="history-item">
      {ticketUsageStatus
        .filter((ticket) => ticket.status === "사용중")
        .map((ticket) => (
          <div key={ticket.ticketName}>
            ...
            <ul className="box-min">
              <li>
                ...
                {/* <p className="date">{ticket.startDate}~{format(addDays(new Date(ticket.startDate), ticket.period), 'yyyy/MM/dd')}</p> */}
              </li>
              <li>
                <p className="tit">잔여횟수</p>
                <p className="date"> {ticket.remainingCount}회 </p>
              </li>
              <li>
                <p className="tit">잔여일</p>
                <p className="date">{ticket.remainingDays}일</p>
              </li>
              <li>
                <p className="tit">매니저</p>
                <p className="date">{ticket.instructor} 강사님 </p>
              </li>
            </ul>
            <ul className="df">
              <li> <ExclamationCircleOutlined /> <p>{ticket.count}회 이용권</p></li>
              <li> <ExclamationCircleOutlined /> <p>이용횟수 주3회 권장</p> </li>
            </ul>
          </div>
        ))
      }
